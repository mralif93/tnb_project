...
@IonicPage()
@Component({
  selector: 'page-compliance-form-bm',
  templateUrl: 'compliance-form-bm.html',
})
export class ComplianceFormBmPage extends ComplianceSignComponent {
    ...
    switch (this.formType) {
      ...
      case 'notisGanggungan':
        ...
        if (typeof this.itemOri.json.wol4 !== 'undefined' && this.itemOri.json.wol4 !== null && this.itemOri.json.wol4 !== '') {
          ...
          this.tempCeassation.ta4indatetime = moment(this.itemOri.json.wol4).format("DD/MM/YYYY"); //003
          this.formBCess.ta4datetime = moment(this.itemOri.json.wol4).format("DD/MM/YYYY");        //003
          // this.tempCeassation.datetime = moment(this.itemOri.json.actstart).format("YYYY-MM-DDTHH:mmZ");
        } else {
          this.tempCeassation.ta4datetime = '';
      ...
      case 'pengujian_pemeriksaan':
        // setting actual start date
        if (typeof this.itemOri.json.wol4 !== 'undefined' && this.itemOri.json.actstart !== null && this.itemOri.json.wol4 !== '') {
  ...
  */
        ...
        if (formCess == true) {
          ...
          if (this.pdfLanguage == 'bhs') {
            if (this.tempCeassation.ta4agree === true || typeof (this.tempCeassation.ta4agree) == 'undefined') {
              ...
              let formType = 'borang_b';
              let formBTemp: any = this.gv.loc_woComplaints.get('formBCess' + this.wonum);
              let formBData = formBTemp.data;
              loading.present().then(() => {
                this.pdf_Bm.pdfDocumentBhs(this.tempCeassation, null, this.formType, 'eng', formBData, 'no Value', 'no Value')
                  .then(result => {
                    this.pdfObj2 = pdfMake.createPdf(result);
  ...
  formatTimeDisplay() {
    let convertTimeDate, dateformat;
    debugger;
    switch (this.formType) {
      case 'notisGanggungan': {
        ...
        if (this.gv.deviceSignal <= data) {
          ...
          this.dataService.saveSealRecordImage(objCopy, this.itemOri.json.wonum, 'addPdf', '', this.itemOri.json.worktype, this.describedBy.docType)
            .then(results => {
              ...
              this.gf.warningAlert("Compliant", error, "OK");
              loading.dismiss();
            });
        } else {
          this.jsonStore.replaceWO(this.itemOri, "LPCWORKORDER", true);
  ...
  findZoneBySiteId() {
    ...
    this.jsonStore.getAllRecord(Domains.SiteID).then(result => {
      ...
      console.log("ori Site Id : "+this.itemOri.json.siteid);//001
        ...
        //if (typeof (this.siteId_region) !== "undefined" && this.siteId_region !== null && this.siteId_region !== "") { //001
          ...
          switch (this.formType) {
            ...
            case 'borangA': {
              ...
              this.siteId_region = "TNB" + " " + itemId[0].json.description;
              console.log("this.siteId_region : "+this.siteId_region);
            } break;
            case 'pepasanganMeter': {
          ...
          if (this.pdfLanguage == 'bhs') {
            ...
            loading.present().then(() => {
              ...
              this.pdf_Eng.pdfDocumentEng(this.formCust, null, this.formType, 'eng', this.deliver, this.disconnect, this.reconnect).then(result => {
                ...
                this.downloadPdf(this.pdfObj, "FormA Staff");
