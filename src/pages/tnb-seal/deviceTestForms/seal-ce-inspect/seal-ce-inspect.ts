...
import { IonicPage, NavController, NavParams, Platform, ViewController, App, LoadingController, ToastController } from 'ionic-angular';

declare let mobilesignalswift;

@IonicPage()
@Component({
  selector: 'page-seal-ce-inspect',
  templateUrl: 'seal-ce-inspect.html',
})
export class SealCeInspectPage {
  ...
  saveDetails() {
    ...
    if (this.gv.testMobile && (DeviceConstants.NETWORK_UNKNOWN === this.gf.checkNetwork() || DeviceConstants.NETWORK_NONE === this.gf.checkNetwork())) {
      ...
      mobilesignalswift.getSignalStrength((data) => {
