...
import { b4 } from '@angular/core/src/render3';
...
@IonicPage()
@Component({
  selector: 'page-sil-sticker-info',
  templateUrl: 'sil-sticker-info.html',
})
export class SilStickerInfoPage {
  ...
  showAddCommModule: boolean = false;
  ...
  public disableBefore: boolean = false;
...
debugger;
console.log(">SilStickerInfoPage >>constructor");
this.loadingPresent();
  ...
  userInputLengthNum(eventData, nameText, indexArray, type) {
    ...
    switch (nameText) {
      ...
      case 'OptEye':
        ...
        break;
      case 'CommModule':
        const RegExpCommModule = /^[-a-zA-Z0-9@$&_*/\\]*$/;
        let newValueCommModule = eventData.target.value;
        var newVal2;
        let regExpCommModule = new RegExp(RegExpCommModule);
  ...
  replaceMeterNCheck() {
    ...
    if (checkingDeviceSelect === DeviceConstants.BCRM_EXISTING_INDICATOR_MAIN) {

      var mmeter = multiassetlocci_temp.filter((item) => {
    ...
    } else if (checkingDeviceSelect === DeviceConstants.BCRM_EXISTING_INDICATOR_CHECK) {

      var cmeter = multiassetlocci_temp.filter((item) => {
        return item.ta0bcrmuploadindicator === DeviceConstants.BCRM_NEW_INDICATOR_CHECK;
      });
